...
- Contributor review pages paginate through
  `POST /maps/_/MapsWizUi/data/batchexecute`. Being a POST, the pagination token
  lives in the body, so there is no URL to diff and replay.
...
- **The default run timeout is 300 s** and a run does roughly one contributor
  every 6 s, so anything past ~50 people dies half-finished with a `TIMED-OUT`
  status and a partially filled dataset. Pass `?timeout=3600` on the run URL.
